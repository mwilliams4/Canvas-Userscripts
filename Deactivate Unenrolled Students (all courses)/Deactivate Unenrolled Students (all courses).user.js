...
(function () {
  ...
  const courseTerms = [
    6240, 6280, 6225, 6255, 6285, 6215, 6245, 6275, 6200, 6211, 6260, 6231,
    6232, 6233,
  ...
  function addButton() {
    ...
    parent = document.querySelector(
      "#right-side > div.events_list.recent_feedback + div"
    );
    if (!parent) return;
  ...
  async function processRequest() {
    ...
    const coursesWithManualSections = identifyCoursesWithManualSections(
      courses,
      sections
    );

    // ### Exporting list of courses with manual sections
    const filteredCoursesWithManualSections = coursesWithManualSections.map(
      (course) => {
        return {
          name: course.name,
    ...
    csvArr.push(
      Object.keys(filteredCoursesWithManualSections[0]),
      ...filteredCoursesWithManualSections.map((course) =>
    ...
    console.table(coursesWithManualSections, [
      "name",
      "sis_course_id",
      "sections",
    ]);
    if (
      !confirm(`Found ${coursesWithManualSections.length} courses with manual sections. See console for details.
Continue?`)
    );
    ...
    for (let i = 0; i < coursesWithManualSections.length; i++) {
      ...
      console.log(
        `Now running ${course.name}.\n(course ${i + 1} of ${
          coursesWithManualSections.length
        })`
      );
      ...
      unenrolledStudents = identifyUnenrolledStudents(
        sectionEnrolments,
        course
      )[1];
      ...
      if (unenrolledStudents.length === 0) {
        console.log(
          `Could not find any unenrolled students for ${course.name}.`
        );
        continue;
      }
      ...
      table.forEach((unenrolledStudent) => {
        unenrolledStudent.sections = unenrolledStudent.sections
          .map((section) =>
            section.sectionSISId ? section.sectionSISId : "null"
          )
          .join();
      });
      ...
      if (
        confirm(`Found ${unenrolledStudents.length} unenrolled students in ${course.name}.
Check console for details.
Continue?`)
      ) {
        logs = [
          ...logs,
          ...(await deactivateStudents(unenrolledStudents, course, token)),
        ];
      }
    }
  ...
  async function deactivateStudents(unenrolledStudents, course, token) {
    ...
    for (let i = 0; i < unenrolledStudents.length; i++) {
      if (
        unenrolledStudents[i].sections &&
        unenrolledStudents[i].sections.length === 0
      )
        continue;
      ...
      for (let j = 0; j < unenrolledStudents[i].sections.length; j++) {
        ...
        if (!response.ok) {
          console.log(
            `Failed to deactivate ${student.studentId} in section ${section.sectionSISId}`
          );
          deactivated = "failed";
        } else {
          console.log(
            `Deactivated ${student.studentId} in section ${section.sectionSISId}`
          );
          deactivated = "success";
        }
    ...
    enrolments.forEach((enrolment) => {
      ...
      if (
        !sectionEnrolments
          ...
          .includes(enrolment.sis_user_id)
      ) {
  ...
  function identifyCoursesWithManualSections(courses, sections) {
    ...
    courses.forEach((course) => {
      ...
      course.sections = sections.filter(
      ...
      course.hasOCMSections = course.sections.find((section) => {
        if (
          ...
          (section.sis_section_id.includes(OCMCohortSectionSubstring) ||
            section.sis_section_id.includes(OCMDefaultSectionSubstring))
        )
          return true;
      })
        ? true
    ...
    return courses.filter(
      (course) =>
        course.hasManualSections === true && course.hasOCMSections === true
    );
  }
  ...
  async function getSections(courses) {
    ...
    while (flag === false) {
      urls = courses
        .slice(
          iteration * asyncRequests,
          iteration * asyncRequests + asyncRequests
        )
        .map(
          (course) =>
            `${window.location.origin}/api/v1/courses/${course.id}/sections?per_page=100`
        );
      ...
      try {
        ...
        console.log(
          `Fetched from courses ${iteration * asyncRequests + 1} to ${
            iteration * asyncRequests + asyncRequests
  ...
  async function getCourses() {
    ...
    courseSubAccounts.forEach(
      (subAccount) =>
        (subAccountParams += `&by_subaccounts[]=sis_account_id:${subAccount}`)
    );
    const standardParams =
      "with_enrollments=true&enrollment_type[]=student&blueprint=false&include[]=term&per_page=100";
