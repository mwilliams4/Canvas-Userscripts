...
(function () {
  ...
  function addButtons() {
    var parent;
    if (
      document.getElementById(courseEnrolmentsLinkId) &&
      document.getElementById(sectionEnrolmentsLinkId)
    )
      return;
    ...
    anchor.id = courseEnrolmentsLinkId;
    ...
    anchor.appendChild(document.createTextNode(` Export course users`));
    listItem.appendChild(anchor);
    parent.appendChild(listItem);

    // Section enrolments button
    var listItem = document.createElement("li");
    listItem.setAttribute("role", "presentation");
    listItem.classList.add("ui-menu-item");
    listItem.style.setProperty("cursor", "pointer");
    ...
    icon.setAttribute("aria-hidden", "true");
    anchor.appendChild(icon);
    anchor.appendChild(document.createTextNode(` Export section enrolments`));
    listItem.appendChild(anchor);
    parent.appendChild(listItem);
    return;
  }

  async function processRequest(sections) {
    ...
    switch (sections) {
        ...
        filteredEnrolments = enrolments.map((enrolment) => {
          ...
          for (let i = 0; i < enrolment.enrollments.length; i++) {
            userEnrolments.push({
              id: enrolment.id,
              created_at: new Date(
                enrolment.enrollments[i].created_at
              ).toLocaleString(),
              name: enrolment.name,
              sis_user_id: enrolment.sis_user_id,
              email: enrolment.email,
              sis_section_id: enrolment.enrollments[i].sis_section_id
                ? enrolment.enrollments[i].sis_section_id
                : "",
            });
          }
      ...
      case false:
        filteredEnrolments = enrolments.map((enrolment) => {
          return {
            id: enrolment.id,
            created_at: enrolment.enrollments // Get earliest section enrolment
              .map((enrolment) => new Date(enrolment.created_at))
              .reduce((a, b) => {
                return a > b ? b : a;
              })
              .toLocaleString(),
    ...
    csvArr.push(
      headers,
      ...filteredEnrolments.map((enrolment) => Object.values(enrolment))
    );
